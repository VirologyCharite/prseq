...
def ensure_data_exists(data_dir: Path, sequences: int, line_length: int, seed: int | None,
                       seq_min: int, seq_max: int) -> tuple[Path, Path]:
...
def print_results(fasta_results: list, fastq_results: list):
    ...
    if fasta_results:
        print("\nFASTA Benchmarks:")
        print("-" * 120)
        print(f"{'Implementation':<20} {'Sequences':>12} {'Bases':>15} {'Time (s)':>10} {'Throughput':>15} {'% of C':>10} {'Slowdown':>10}")
        print("-" * 120)

        # Get C baseline throughput (first result should be C)
        c_throughput = fasta_results[0].get('throughput', 0) if fasta_results and fasta_results[0] else 0
    ...
    if fastq_results:
        ...
        # Get C baseline throughput (first result should be C)
        c_throughput = fastq_results[0].get('throughput', 0) if fastq_results and fastq_results[0] else 0
    ...
    if fasta_results and rust_fasta_pct > 0:
        print(f"- Rust/Python (prseq): {rust_fasta_pct:.1f}% of C speed for FASTA, "
...
def main():
    ...
    parser.add_argument(
        "--fasta-only",
        action="store_true",
        help="Run only FASTA benchmarks"
    )
    parser.add_argument(
        "--fastq-only",
        action="store_true",
        help="Run only FASTQ benchmarks"
    )
    parser.add_argument(
        "--skip",
        action="append",
    ...
    # Print results
    print_results(fasta_results if not args.fastq_only else [],
